                                    ...
                                    QWidget* parent ) : ADeviceWidget( parent )
{
  ...
  mBatteryLevelBar = new CDataBar( mBatteryBox, "Level [%]", 0, 1000 );
  batteryLayout->addWidget( mBatteryLevelBar );
...
void CPowerPackWidget::updateData()
{
  if ( isChecked() ) {
    ...
    if ( mBatteryBox->isChecked() ) {
      ...
      mBatteryLevelBar->setHidden( false );
      ...
      mBatteryLevelBar->setData( mPowerPack->getBatteryLevel() * 1000);
    }
    else {
      ...
      mBatteryLevelBar->setHidden( true );
    }
